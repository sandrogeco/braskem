...
while 1<2:
    ...
    for s in np.arange(0, ns):
        traces[s] = client.get_waveforms('LK', st[s], '', 'EH?', tt - lta * wnd, tt + lta * wnd)
    ...
    for s in np.arange(0, ns):
        ...
        vpp[s] =np.max(obspy.signal.filter.envelope(tr[s][0].data))#np.max(trAmpl(tr[s])) #np.max(np.abs(tr[s].data))
