...
class alert():
    ...
    def getAlerts(self,ts,te,station='', event_type=''):
        try:
            ...
            "' AND utc_time<'"+UTCDateTime(te).strftime("%Y-%m-%d %H:%M:%S")+"';"
    ...
    def HR_run(self,st,te=UTCDateTime.now()):
        while 1<2:
            if te<UTCDateTime.now():
                for s in st:
                    print(te)
                    if self.hourlyRate(te, s, 'AML'):
                        self.RA_alert()
                    if self.hourlyRate(te, s, 'AMH'):
                        self.RA_alert()
    ...
    def RA_alert(self):
        r=False
        # aa=self._a
        # ampl=np.max([aa['amplitude_ehz'],aa['amplitude_ehe'],aa['amplitude_ehn']])
        # fR=np.where(self._rateX>=aa['rate'])[0]
        # fA=np.where(self._amplY<=ampl)[0]
        # fR=fR[0]
        # fA=fA[0]
        # r=self._thMatrix[fA,fR]
        # if r>0:
        #
        #     self._a['utc_time'] = aa['utc_time']
        #     self._a['utc_time_str'] = aa['utc_time_str']
        #     self._a['event_type'] = "'RA_" +aa['event_type'].replace("'",'')+ "'"
        #     self._a['station'] = aa['station']
        #     self._a['amplitude_ehe'] = aa['amplitude_ehz']
        #     self._a['amplitude_ehn'] = aa['amplitude_ehn']
        #     self._a['amplitude_ehz'] = aa['amplitude_ehz']
        #     self._a['rate'] =aa['rate']
        #     self._a['level']=r
        #     self.insert()
        return r
...
#            plt.close(im)
    ...
    def run(self, network, station, channel,rt=True):
        ...
        while 1 < 2:
            ...
            fileLast.write(UTCDateTime(self._lastData).strftime("%Y-%m-%d %H:%M:%S"))
            fileLast.close()
